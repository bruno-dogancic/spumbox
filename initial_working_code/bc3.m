...
% generating static filter and static delta for the nominal system
for j = 1:ns
...
% Non structure-preserving result
% Using the finest discretization make the interconnected system
% G_old = G(end).agg;
G_old = ss(G(end).agg);
% To obtain comparable results to the new procedure, we compare the nominal
% system to the ones that has static filters
ord_old = size(Mrs.A,1);
[Gr_old,~] = balred(G_old,ord_old,balred_opts);

% Making of the weigths
G_rel_old = G_old - Gr_old;
% W1
W1frd = frd(G_rel_old,wlog);
W1frda = abs(W1frd);
W1 = rss(z(1),size(G_rel_old.y,1),size(G_rel_old.u,1)); % (to compare with the first filter order of W1,1)
for jj = 1:size(G_rel_old.y,1)
    for kk = 1:size(G_rel_old.u,1)
        C1.UpperBound = [];
        C1.LowerBound = W1frda(jj,kk);
        b1 = fitmagfrd(W1frda(jj,kk),z(1),[],W1frda(jj,kk),C1);
        W1(jj,kk) = b1;
    end
end
