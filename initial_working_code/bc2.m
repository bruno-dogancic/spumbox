...
x = [nd, 9, 8, 7, 6, 4, 3];%, 4, 4, 3];%

msord = 20; % maximum order of the reduced subsystems
y = [6, 3, 3, 2, 2, 2, 2];%, 2, 2, 1];% nb = 10, 
...
zz = [0, 3, 2, 2, 2, 1, 1];%, 2, 1, 1];% % for all systems except first

% Vector for choosing orders of Ww
zw = [0, 3, 2, 2, 2, 1, 1];%, 1, 1, 1];% % for all systems except first
...
de = iqcassign(de,'ultid','Length',2,'PoleLocation',-10, 'BasisFunctionType',1);
...
%         probr = iqcanalysis(Mr,{de,pe},'SolChk','on','eps',1e-8,'Parser','Yalmip','Solver','mosek');
% toc
...
%         if mod(j,2) == 1
%             figure()
%             bodemag(G(j).W1,'k',G(j).W1e,'-.r',G(j).W1s,'--g')
%         end
%     end
% end
